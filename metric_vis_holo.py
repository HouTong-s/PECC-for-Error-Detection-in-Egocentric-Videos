    ...
    for i in range(len(gt)):
        ...
        if pre_gt != gt[i]:
            if is_pred_error and is_pre_gt_error and num_error > num_nonerror:
                num_correct += 1
            elif (not is_pred_error and not is_pre_gt_error) or (is_pred_error and not is_pre_gt_error and num_error < num_nonerror):
...
# 读取txt文件并将其转化为action2idx格式
def load_action_mappings(txt_file):
    action2idx = {}
    with open(txt_file, 'r') as f:
        for line in f:
            idx, action_name = line.strip().split(' ')
            action2idx[action_name] = int(idx)
...
class ActionSegmentationErrorDetectionEvaluator:
    def __init__(self, args):
        ...
        root_dir = f'./data/HoloAssist'
        with open(os.path.join(root_dir,  'test.txt'), 'r') as fp:
            lines = fp.readlines()
            self.data_list = [line.strip('\n') for line in lines]
        with open(os.path.join(root_dir, 'annotation_10fps.json'), 'r') as fp:
        ...
        annot = all_annot
        self.action = str(args.action)+"_action"
        for i in tqdm(range(len(annot)), desc='Processing segments'):
            video_id = annot[i]['video_id']
            if video_id in self.data_list:
                actions = [int(action) for action in annot[i]['labels'][self.action]]
                action_types = [int(action_type) for action_type in annot[i]['labels']['action_type']]
                self.annotations[video_id] = [np.array(annot[i]['labels']['time_stamp']) * args.default_fps, 
                                              np.array(actions),
                                              np.array(action_types)]
        
        
        txt_file = f"{args.action}_index.txt"  # 替换为你的txt文件路径
        action2idx = load_action_mappings(os.path.join(root_dir,  txt_file))
    ...
    def macro_segment_error_detection(self, output_list=None, threshold=None, is_visualize=False):
        ...
        with open(os.path.join('ckpt', self.args.dirs, self.args.dirname, "pred_seg_results_%.2f.pkl"%(threshold)), "rb") as f:
        ...
        for video_id in self.data_list:
            ...
            gt_segments, gt_labels, gt_label_types = self.annotations[video_id]
    ...
    def micro_framewise_action_segmentation(self, eval_each_class=True, is_visualize=False):
        ...
        with open(os.path.join('ckpt', self.args.dirs, self.args.dirname, 'eval_results.pkl'), "rb") as f:
            results = pickle.load(f)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default='HoloAssist')
    parser.add_argument('--dirs', type=str, default='HoloAssist')
    parser.add_argument('--action', type=str)
    parser.add_argument('--fps', default=10, type=int)
    parser.add_argument('--split', type=str, default='test')
    parser.add_argument('--default_fps', type=int, default=10)
    ...
    if args.error_detection:
        if args.threshold != -100.0:
            ...
            fprs = []
            tprs = []
            for threshold in tqdm(range(0, 41), desc="Testing thresholds", unit="threshold"):
            ...
            np.save(os.path.join('./ckpt', args.dirs, args.dirname, 'micro_fpr_tpr.npy'), np.array(micro_fprs_tprs))
            np.save(os.path.join('./ckpt', args.dirs, args.dirname, 'macro_fpr_tpr.npy'), np.array(macro_fprs_tprs))
            np.save(os.path.join('./ckpt', args.dirs, args.dirname, 'eda.npy'), np.array(eda_list))
