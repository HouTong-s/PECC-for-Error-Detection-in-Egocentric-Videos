...
if __name__ == '__main__':
    ...
    parser.add_argument('--dirs', type=str, default='EgoPER')
    ...
    if args.error_detection:
        if args.threshold != -100.0:
            ...
            # After computing eda_list, error_micro_list, and error_macro_list

            micro_fprs = np.array(error_micro_list['fpr'])
            eda_values = np.array(eda_list)

            # Sort fprs and eda_values based on fprs for AUC calculation
            sorted_indices = np.argsort(micro_fprs)
            sorted_fprs = micro_fprs[sorted_indices]
            sorted_eda_values = eda_values[sorted_indices]

            # Ensure the values are between 0 and 1 by appending 1.0 for a closed curve
            sorted_fprs = np.concatenate((sorted_fprs, np.array([1.0])), axis=0)
            sorted_eda_values = np.concatenate((sorted_eda_values, np.array([0.0])), axis=0)

            # Calculate EDA-AUC
            eda_auc_value = auc(sorted_fprs, sorted_eda_values)

            print('|%s|EDA: %.1f|EDA-AUC: %.1f|Micro AUC: %.1f|Macro AUC: %.1f|' % 
                (args.dirname, np.array(eda_list).mean() * 100, eda_auc_value * 100, micro_auc_value * 100, macro_auc_value * 100))
