...
from .models import register_blocks
...
class GraphAttentionLayer(nn.Module):
    """
    A simple Graph Attention Layer
    """
    def __init__(self, in_features, out_features, dropout, alpha, concat=True):
        ...
        # 使用nn.Linear替代self.a，注意这里的in_features应该是2*out_features
        self.attention_layer = nn.Linear(2*out_features, 1)
        nn.init.xavier_uniform_(self.attention_layer.weight, gain=1.414)
        # 注意：通常不初始化偏置为0，但如果你想要可以手动设置self.attention_layer.bias.data.zero_()

    def forward(self, input):
        h = self.linear_layer(input)  # h.shape: (N, out_features)
        
        a_input = self._prepare_attentional_mechanism_input(h)
        e = self.leakyrelu(self.attention_layer(a_input))  # 注意这里不需要squeeze操作
    ...
    def _prepare_attentional_mechanism_input(self, h):
        ...
        # 你需要根据你的具体需求来实现这个函数
        N = h.size()[0]
        node_features = h.repeat(1, N).view(N * N, -1)  # 重复每个节点的特征N次
        return node_features

@register_blocks("GCNAttn")    
class GCNAttetionBlock(nn.Module):
    """
    A simple Graph Convolutional Block with Attention Mechanism
    """
    def __init__(
        self,
        n_embd,                # dimension of the input features
        ...
        self.attn_weight1 = nn.Linear(feat_dim, feat_dim)
        self.attn_weight2 = nn.Linear(feat_dim, feat_dim)
        self.attn_scale = 1.0 / (feat_dim ** 0.5)  # Scaling factor for attention scores

    def generate_graph_feature(self, bbox, bbox_class, edge_map):
        B, T, N, C = bbox_class.size()
        
        # Step 1: Concatenate bbox and bbox_class, and apply convolution
        node = torch.cat((bbox, bbox_class), dim=3).permute(0, 3, 1, 2)  # (B, C, T, N)
        node = self.conv2d_1_weight(node).permute(0, 2, 3, 1)  # (B, T, N, C)
        
        # Step 2: Apply attention mechanism to the edge map
        # Compute attention scores for each edge
        attn_query = self.attn_weight1(node)  # (B, T, N, C)
        attn_key = self.attn_weight1(node).permute(0, 1, 3, 2)  # (B, T, C, N)
        attn_scores = torch.einsum('btik,btkj->btij', attn_query, attn_key)  # (B, T, N, N)
        ...
        # Step 3: Element-wise multiply the attention weights with the original edge_map
        weighted_edge_map = edge_map * attn_weights
        
        # Step 4: Apply graph convolution using the weighted edge map
        node = torch.einsum('btij,btjk->btik', weighted_edge_map, node)
        node = self.relu(node).permute(0, 3, 1, 2)  # (B, C, T, N)
        node = self.conv2d_2_weight(node).permute(0, 2, 3, 1)  # (B, T, N, C)

        attn_query = self.attn_weight2(node)  # (B, T, N, C)
        attn_key = self.attn_weight2(node).permute(0, 1, 3, 2)  # (B, T, C, N)
        attn_scores = torch.einsum('btik,btkj->btij', attn_query, attn_key)  # (B, T, N, N)
